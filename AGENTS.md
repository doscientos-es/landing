# Landing UI guardrails

Apply the Web Interface Guidelines to every public landing change.
...
- Links use `<a>`/Astro `<Link>`; controls use `<button>`; interactive targets are at least 44px on mobile.
- No disabled paste, browser zoom, or incomplete-form submit prevention. Errors are adjacent to fields and async updates use `aria-live`.
- Loading labels keep their action text and use `…`; destructive actions confirm or offer undo.
- Motion has a `prefers-reduced-motion` path, uses explicit transitions, and does not rely on JS for essential content.
- Responsive layout is checked at mobile, laptop, and ultra-wide widths; images reserve dimensions and overflow does not create useless scrollbars.
- Copy is concise, active, Spanish-first, and uses `…`, curly quotes, locale-aware formats, and text alternatives for color/icon cues.

For landing work, run the project checks from this directory and inspect the rendered page at the affected responsive widths. Do not claim remote or authenticated behavior based only on a local build.
